...
用法示例：
  ...
  python run_daily_workflow.py --update-form

  # 指定日期
  python run_daily_workflow.py --signal-date 20260119 --end-date 20260120 --update-form

  python run_daily_workflow.py --signal-date 20260119 --end-date 20260116 --update-form --wecom-webhook "https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=de57fc57-503b-4b2d-b62f-f5a0cb92bd59"
...
def main() -> int:
    ...
    parser.add_argument("--update-form", action="store_true", help="运行 select_stocks_local.py 并写入/增量更新维护表单")
    ...
    parser.add_argument(
        "--wecom-webhook",
        default="",
        help="企业微信机器人 webhook 完整URL（例如 https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=xxx）。不传则不发送。",
    )
    ...
    # 2) 选股（并可更新维护表单）
    select_cmd = [sys.executable, os.path.join(ROOT, "select_stocks_local.py"), "--end-date", signal_date]
    if args.update_form:
        select_cmd.append("--update-form")
    ...
    # 发送企业微信
    if str(getattr(args, 'wecom_webhook', '')).strip():
        content = "\n".join([x for x in msg_lines if str(x).strip()])
        try:
            errcode, errmsg = _send_wecom_text(str(args.wecom_webhook).strip(), content)
            if errcode == 0:
                pass
            else:
