...
def _normalize_query_text(q: Any) -> str:
    """Normalize query string for similarity check.

    Keep it lightweight: lowercase + remove whitespace/punctuation that often changes.
    """
    if q is None:
        return ""
    s = str(q).strip().lower()
...
def _query_similarity_ratio(a: Any, b: Any) -> float:
    """Return similarity ratio in [0,1]."""
    import difflib

    sa = _normalize_query_text(a)
    sb = _normalize_query_text(b)
    if not sa or not sb:
        return 0.0
    return difflib.SequenceMatcher(None, sa, sb).ratio()


def _is_query_similar(
    query: Any,
    seen_queries: list[str],
    *,
    threshold: float,
) -> tuple[bool, float, str | None]:
    """Check if query is similar to any seen query.

    Returns: (is_similar, best_ratio, best_match_query)
    """
    qn = _normalize_query_text(query)
    if not qn:
    ...
    # Compare against normalized seen queries
    for sq in seen_queries:
        if not sq:
            continue
        r = _query_similarity_ratio(qn, sq)
        if r > best_ratio:
            best_ratio = r
            best_match = sq
...
def _replay_from_scanned_records(
    ...
    *,
    ...
    for r in scanned_records:
        ...
        if qsim_th is not None and query:
            is_sim, best_ratio, _ = _is_query_similar(query, seen_queries_norm, threshold=float(qsim_th))
            if is_sim:
                compare_rows.append(
                    {
                        "strategy_id": sid,
                        ...
                        "replay_mwr1_num": mwr1_num,
                        "replay_pass": False,
                        "replay_reason": "query_similarity",
                        "replay_query": query,
                        "query_sim_best_ratio": best_ratio,
                        "query_sim_pass": False,
                    }
                )
                continue
        ...
        if qsim_th is not None and query:
            seen_queries_norm.append(_normalize_query_text(query))
    ...
    *,
    ...
    for idx, sid in enumerate(strategy_ids, start=1):
        ...
        if qsim_th is not None and query:
            ...
            if is_sim:
                print(
                    f"跳过[manual]：query 相似度>= {float(qsim_th):.2f} sid={sid_int} best_ratio={best_ratio:.3f}"
                )
                continue
...
def _collect_rows_for_sort(
    ...
    *,
    ...
    filtered_out_query_similarity = 0
    ...
    while page <= max_pages and len(final_rows) < min_output_count:
        ...
        for it in items:
            ...
            # Query similarity filter
            if qsim_th is not None:
                is_sim, best_ratio, _ = _is_query_similar(query, seen_queries_norm, threshold=float(qsim_th))
                if is_sim:
                    filtered_out_query_similarity += 1
                    continue
    ...
    print(
        ...
        f"query相似度过滤剔除={filtered_out_query_similarity}, "
...
def main(
    *,
    ...
    # query 相似度过滤阈值（0~1），默认 0.70；设为 0 或 None 表示关闭
    query_similarity_threshold_raw = _cfg_get(cfg, ["filters", "query_similarity_threshold"], 0.70)
    try:
        qst = float(query_similarity_threshold_raw)
    except Exception:
        qst = 0.70
    query_similarity_threshold = None if qst <= 0 else max(0.0, min(1.0, qst))
